				...
				<div class="content-bill">
					...
					<div class="grid mt-5 mb-5" style="display:grid; grid-template-columns: repeat(2, 1fr);">
						...
						<div class="g-col-6">
							...
							<div class="mt-1 trang_thai_<?= $trang_thai ?>">
								<?= $ten_trang_thai ?>
							</div>
						</div>
					</div>
					...
					<div class="flex justify-content-between mt-3">
						<div class="btn_ct" id="delete" style="cursor: pointer;">Huỷ đơn</div>
...
<script>
	$('#delete').click((e) => {
		var ma_don_hang = getParameterByName('madonhang')

		var form_data = new FormData();
		form_data.append('ma_don_hang', ma_don_hang);

		$.ajax({
			url: "api/main.php?act=getTrangThai", //Server api to receive the file
			type: "POST",
			cache: false,
			contentType: false,
			processData: false,
			...
			success: function(suc) {
				var trangthai = JSON.parse(suc)
				trangthai = trangthai['trang_thai']
				if (trangthai > 1) {
					toast({
						title: "Thất bại!",
						message: "Đơn hàng đã được xác nhận, muốn huỷ đơn xin vui lòng liên hệ quản trị viên",
						type: "error",
						duration: 5000
					});
				} else {
					form_data.append('trang_thai', 6);
					$.ajax({
						...
						success: function(suc) {
							toast({
								title: "Thành công!",
								message: "Bạn đã huỷ đơn hàng thành công",
								type: "success",
								duration: 5000
							});
							setTimeout(() => {
